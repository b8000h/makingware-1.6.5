...
class Mage_Newsletter_Model_Template extends Mage_Core_Model_Template {
	...
	public function isValidForSend() {
		return Mage::getStoreConfigFlag ( 'system/smtp/email_agent' ) && $this->getTemplateSenderName () && $this->getTemplateSenderEmail () && $this->getTemplateSubject ();
	}
	...
	public function getInclude($templateCode, array $variables) {
		return Mage::getModel ( 'newsletter/template' )->loadByCode ( $templateCode )->getProcessedTemplate ( $variables );
	}
	...
	public function send($subscriber, array $variables = array(), $name = null, Mage_Newsletter_Model_Queue $queue = null) {
		...
		if ($subscriber instanceof Mage_Newsletter_Model_Subscriber) {
			...
			if (is_null ( $name ) && $subscriber->hasCustomerName ()) {
