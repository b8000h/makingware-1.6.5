<?php
class Makingware_Cms_Block_Adminhtml_Page_Edit_Tab_Design 
    ...
    implements Mage_Adminhtml_Block_Widget_Tab_Interface
{
    ...
    protected function _prepareForm()
    {
        ...
        if (!$model->getId()) {
        	...
        	if ($id = $this->getRequest()->getParam('parent')) {
        		$parentModel = Mage::getModel('cms/page')->load($id);
        		if ($parentModel->getId() && $parentModel->getRootTemplate()) {
        			$model->setRootTemplate($parentModel->getRootTemplate());
        		}
        	}
        }
